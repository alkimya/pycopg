...
pycopg provides a `db.backup.*` (and `async_db.backup.*`) accessor namespace with
convenient wrappers around PostgreSQL's backup and restore tools.
...
# Sync: db.backup is initialized lazily on first access
...
# Async: async_db.backup mirrors the sync API with awaited methods
...
db.backup.pg_dump("backup_dir", format="directory")

# Tar format
db.backup.pg_dump("backup.tar", format="tar")
```
...
db.backup.pg_dump("data.dump", data_only=True)
```
...
db.backup.pg_dump("users.dump", tables=["users", "user_profiles", "user_settings"])

# Exclude tables
db.backup.pg_dump("backup.dump", exclude_tables=["logs", "sessions", "temp_data"])
```
...
db.backup.pg_dump("app.dump", schemas=["app", "auth"])
```
...
db.backup.pg_restore("backup_dir")
```

### Restore Options

```python
db.backup.pg_restore(
    "backup.dump",
...
db.backup.pg_restore("backup.dump", clean=True, if_exists=True)
```
...
db.backup.pg_restore("backup_dir", jobs=4)
```
...
db.backup.pg_restore("backup.dump", tables=["users", "orders"])
```
...
db.backup.pg_restore("backup.dump", no_owner=True, no_privileges=True)
```
...
```python
db.backup.pg_restore("backup.sql")  # Automatically uses psql for .sql files
```
...
```python
# Basic export
rows = db.backup.copy_to_csv("users", "users.csv")
print(f"Exported {rows} rows")

# With options
db.backup.copy_to_csv(
    "users",
    "users.csv",
    schema="public",
...
```python
# Basic import
rows = db.backup.copy_from_csv("users", "users.csv")
print(f"Imported {rows} rows")

# With options
db.backup.copy_from_csv(
    "users",
    "users.csv",
    schema="public",
...
db.backup.pg_dump(f"backups/dev_{timestamp}.dump")

# Keep only schema in version control
db.backup.pg_dump("migrations/schema.sql", format="plain", schema_only=True)
...
def clone_database(source_url: str, target_url: str):
    ...
    try:
        ...
        source_db.backup.pg_dump(backup_file)

        # Restore to target (clean = drop existing objects)
        print("Restoring to target database...")
        target_db.backup.pg_restore(backup_file, clean=True, no_owner=True)
...
db.backup.pg_dump("backup.dump", format="custom")
```
...
db.schema.create_database("test_restore")

# Restore to test
test_db = Database(db.config.with_database("test_restore"))
test_db.backup.pg_restore("backup.dump")
...
# Cleanup
test_db.close()
db.schema.drop_database("test_restore")
```
...
- name: Backup before deploy
  run: |
    ...
    from pycopg import Database
    db = Database.from_env()
    db.backup.pg_dump('pre_deploy_backup.dump')
    "
```
...
db.backup.pg_dump(str(backup_file))
