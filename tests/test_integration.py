...
class TestIntegration:
    ...
    def test_authors_table(self, db_config):
        ...
        rows = db.execute(
            "INSERT INTO test_schema.authors (name, email) VALUES (%s, %s) RETURNING id",
            ["Dave", email],
        )
        new_id = rows[0]["id"]
        ...
        # Select
        authors = db.execute(
            "SELECT * FROM test_schema.authors WHERE name = %s AND email = %s",
            ["Dave", email],
        )
    ...
    def test_transaction_rollback(self, db_config):
        ...
        db = Database(db_config)
        initial_count = db.execute("SELECT count(*) as c FROM test_schema.authors")[0][
            "c"
        ]
        ...
        try:
            with db.transaction() as conn:
                ...
                with conn.cursor() as cur:
                    cur.execute(
                        "INSERT INTO test_schema.authors (name, email) VALUES (%s, %s)",
                        ["Eve", email],
                    )
        ...
        final_count = db.execute("SELECT count(*) as c FROM test_schema.authors")[0][
            "c"
        ]
