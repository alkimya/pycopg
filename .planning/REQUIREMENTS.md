...
- [ ] **SEC-02**: Sync `session()` no longer masks the original exception when commit/close fails in `finally` (B2)
...
- [ ] **SEC-06**: Each residual fix has a dedicated red→green regression test

### Parity — Sync/Async (Phase 11)

- [ ] **PAR-01**: AsyncDatabase implements `add_primary_key`, `add_foreign_key`, `add_unique_constraint`, `truncate_table`
- [ ] **PAR-02**: AsyncDatabase implements `drop_extension`, `database_exists`, `list_databases`, `create`/`create_from_env`
- [ ] **PAR-03**: Database implements `insert_many`, `upsert_many`, `stream`, `notify`
- [ ] **PAR-04**: Async `from_dataframe`/`from_geodataframe` actually apply `primary_key` instead of silently ignoring it (C1)
- [ ] **PAR-05**: `AsyncDatabase.close()` disposes the SQLAlchemy async engine instead of being a no-op (C2)
- [ ] **PAR-06**: Async engine uses the `postgresql+psycopg_async://` driver URL (C3)
- [ ] **PAR-07**: `create_extension(schema)` and `create_schema(owner)` signatures and `table_info`/`list_roles` semantics aligned sync/async
- [ ] **PAR-08**: `test_parity` verifies return fields and mocked behavior, not just method/param names
- [ ] **PAR-09**: Coverage ratchet raised to 90% (`--cov-fail-under=90`) and passing
...
- [ ] **REF-02**: `Database` and `AsyncDatabase` inherit the shared `base.py` layer (`DatabaseBase`, `QueryMixin`); `from_env`/`from_url`/`__repr__` and batch-insert builder lifted up (R3)
...
- [ ] **REF-04**: Residual dead code removed (unused `import re`, unread `stdout`, no-op `try/except: raise`, unused `*_SIMPLE` constants, stale "Phase 3" comments)
- [ ] **REF-05**: Coverage ratchet raised to 95% (`--cov-fail-under=95`) and passing
...
- [ ] **DOC-07**: `interrogate` added as a dev dependency with `fail-under=95` and enforced in a CI job
- [ ] **DOC-08**: `napoleon_numpy_docstring` enabled in Sphinx conf
...
- [ ] **DOC-10**: `__version__` resolved via `importlib.metadata.version()` so all version sources agree (V1)
- [ ] **DOC-11**: mypy added as a dev dependency with config (TY1)
...
| PAR-07 | Phase 11 | Pending |
| PAR-08 | Phase 11 | Pending |
| PAR-09 | Phase 11 | Pending |
| REF-01 | Phase 12 | Pending |
| REF-02 | Phase 12 | Pending |
| REF-03 | Phase 12 | Pending |
| REF-04 | Phase 12 | Pending |
| REF-05 | Phase 12 | Pending |
| DOC-06 | Phase 13 | Pending |
| DOC-07 | Phase 13 | Pending |
| DOC-08 | Phase 13 | Pending |
| DOC-09 | Phase 13 | Pending |
| DOC-10 | Phase 13 | Pending |
| DOC-11 | Phase 13 | Pending |
