...
- [x] **ALIAS-RM-02**: Calling any removed flat name raises a plain `AttributeError` (no stub, no warning); the alias-specific warn+delegate tests are removed and `test_parity`/`ACCESSOR_PAIRS` still passes green
...
- [x] **ETL-INC-11**: Full sync/async parity — `AsyncETLAccessor` mirrors the entire incremental surface; `test_accessor_parity` (over `ACCESSOR_PAIRS`, the live successor to the removed `TestEtlParity`) covers it
