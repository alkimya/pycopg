...
3. User can call `db.timescale.add_dimension("my_hypertable", "device_id", partition_type="hash", number_partitions=4)` on an empty hypertable and confirm the dimension is registered; calling with the wrong type/param combination raises `ValueError` at construction time; calling on a non-empty hypertable raises a clear pycopg-domain error
4. User can call `db.timescale.add_reorder_policy("my_hypertable", "my_index")` and confirm the policy job row exists in `timescaledb_information.jobs`; `CALL run_job(job_id)` completes without error
