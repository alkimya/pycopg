...
1. **Real PostgreSQL beats mocks** for catching driver/DB interaction bugs (v0.3.0 and v0.4.0 both surfaced latent bugs only real-DB tests caught).
2. **Sync is the established core-value API; align async toward it** — enriching async never breaking sync kept the parity promise with zero sync breakage across both milestones.
